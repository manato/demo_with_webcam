...
img_receive="${base_path}/bin/image_receiver/camera_server"
...
then
    ...
    line_noLineDetect_pid=`ps | grep line_detecter_noLineDetect | awk '{print $1}'`
    ...
    if [ -n "${line_noLineDetect_pid}" ]; then
        kill -KILL ${line_noLineDetect_pid}
    fi
...
then
    ...
    do 
        ...
        case ${arg} in
            ...
            --nopds) pds_detect="";;
            --nosgn) sgn_detect="";;
            --noline) lin_detect=""; img_receive="${base_path}/bin/image_receiver/camera_server_noLineDetect";;
            *);;
        esac
    done
    # execute comands
