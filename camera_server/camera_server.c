/* server program 
   which get image from USB camera
*/
#include <stdio.h>
#include <highgui.h>
...
int main(void){
    ...
    DLimgdescriptor imgdes, imgdes_shm, imgdes_shm_output; /* image descriptor */
    unsigned char *buf;         /* buffer for image from camera */
    unsigned char *buf_output;         /* buffer for image from camera */
    ...
    /* part of get shared memory image for output */
    int shm_image_size;
...
//        cvWaitKey(300);         /* wait 300ms */
        cvWaitKey(2);         /* wait 300ms */

        /* conver image data(unsigned char) to IplImage*/
        shm_image_size = IMAGE_SIZE;
//        tmpbuf = cvCreateMat(1, image_size, CV_8UC3);
